...
namespace BTL_Finance.Controllers
{
    ...
    public class InvoiceController : Controller
    {
        ...
        private readonly IInvoiceService _Invoice_Service;
        ...
        public InvoiceController(UserManager<ApplicationUser> userManager,
            ...
            IInvoiceService Invoice_Service,
        ...
        {
            var result =  _Invoice_Service.GetAllInvoice();
            return View(result);
        ...
        [HttpPost]
        public async Task<IActionResult> AddInvoice(Invoice model, IFormFile file)
        {
            ...
            if (request != null)
            {
                request.process = (int)Enums.EnumProcess.Invoice;
                request.Status_Invoice = true;
                await Request_Service.Update(request);
            }
            //model.RequestFormId= 
            //model.process = (int)Enums.EnumProcess.orderSheet;

            model.CreatedBy = Guid.Parse(user.Id);
            model.LastModifiedBy = Guid.Parse(user.Id);
            bool result = await _Invoice_Service.Insert(model);
            if (result)
            {
                ...
                foreach (var item in List_User_ID)
                {
                    ...
                    n.Title = "Adding Invoice Order";
                    n.Description = "You should Invoice Order sheet Now";
                ...
                if (user != null)
                {
                    ...
                    audit.Type = "Adding Invoice Order";
